...
## 12. 구현 이탈 기록 (03 단계 소급 추가, 2026-06-05)

02 설계 대비 실제 구현이 벗어난 항목을 사실대로 기록한다. 03 을 정식으로 돌리지 않아 그동안 누락됐던 것을 소급 정리.

- **사유는 복기로 채웠다 (2026-06-05)** — 표의 `사유` 칸은 요약 라벨, 상세 규명은 아래 12-1 참조.
- 현재 빌드는 "vertical slice"(앱 타이틀에 명시)다. 일부 `미구현(부재)` 항목(#3~#5)은 후속 슬라이스로 미뤄진 것으로, 그 보류 의도를 12-1에 기록했다.

| # | 항목 | 02 명세 (원래) | 실제 구현 | 상태 | 사유 |
|---|------|---------------|-----------|------|------|
| 1 | 빌드 타깃 이름 | #16: `libplanning_core`, `libplanning_adapter_sqlite / cli / logger / config` 유지(복기 시 재정비) | `planning_core`, `planning_roots / leaves / rings / climate` (식물 테마, `lib` 접두 제거) | 다르게 구현 | 임의 (협의 없음) — 12-1 참조 |
| 2 | UX 패턴 (조작 명령) | 9-1: **(B) 인터랙티브 선택 + 페이지네이션** 채택, (A) ID 직접입력 *기각* | 전 명령 `--id <uuid>` 직접입력 (= 기각됐던 A) | 기각안으로 구현 | #3 보류의 임시 대체물 — 12-1 참조 |
| 3 | 인터랙티브 셀렉터 / FTXUI | 3장·9-2·조립 예시: leaves `InteractiveSelect(FTXUI)`, `FtxuiTodoSelector`, `flows/02-todo-done-interactive` | FTXUI 미도입, Selector 클래스 없음 | 미구현(부재) | 의식적 보류 (초안) — 12-1 참조 |
| 4 | leaves 구조 (ArgParser / Dispatcher) | 3장: leaves 에 `ArgParser(CLI11)` + `Dispatcher` 클래스 | `glass/main.cpp` 인라인 CLI11 파싱, Dispatcher/ArgParser 클래스 없음 (leaves = `CliConflictPrompter` + `CliFormat` 만) | 다르게 구현 | 의식적 보류 (초안) — 12-1 참조 |
| 5 | JSON / JSON Schema 라이브러리 | 9-2: nlohmann/json + json-schema-validator(pboettch) 채택 | 미도입 (nutrients 부재, 코드 미사용) | 미구현(부재) | 의식적 보류 (초안) — 12-1 참조 |

> 따른 항목(이탈 아님): 디렉토리 이름(`seed/stem/.../glass`)·네임스페이스(`planning::*`, `adapter_*::`)는 #16 지침대로 유지됨.
> 추가 이탈 발견 시 본 표에 누적.

### 12-1. 이탈 사유 (복기, 2026-06-05)

armarium code-design 03 스킬 복기 중, 위 5건의 `미상` 사유를 사용자와 함께 규명했다. 두 갈래로 나뉜다.

**A. 협의 없이 흘러간 이탈 (#1, #2)** — G4(임의 결정 금지)/G12(설계 이탈 기록)가 있었다면 그 시점에 잡혔을 항목.

- **#1 빌드 타깃 이름**: 식물 테마를 디렉토리에 적용하면서 빌드 타깃명도 같은 테마로 맞췄고, CMake가 STATIC 라이브러리에 `lib` 접두를 자동으로 붙이므로(`add_library(planning_core)` → `libplanning_core.a`) 타깃명의 `lib`는 중복이라 제거. 기술적으로는 타당하나 사용자와 세부 논의 없이 진행된 임의 결정.
- **#2 UX 패턴 B→A**: "직접 입력하자"고 합의한 적 없음. UUID는 내부 자동 생성 식별자 + 표시용으로만 논의됨(사용자 직접 입력은 부담이라 B 패턴 채택). #3(FTXUI 셀렉터) 보류로 대상을 지목할 수단이 사라지자 그 자리의 임시 대체물로 `--id`가 들어온 것. `--id` 임시 채택 자체와 "UUID 비노출" 의도와의 임시 어긋남은 명시 논의되지 않음.

**B. 초안 우선 의식적 보류 (#3, #4, #5)** — 보류 자체는 정당하나, 그 보류 결정이 어디에도 기록되지 않아 소급 발견 전까지 "조용한 이탈"처럼 보였다.

- **#3 FTXUI 셀렉터**: 인터랙티브 셀렉터(B 패턴)를 후속 슬라이스로 미루기로 의식적 결정. 우선 단순 경로로 포트를 채우고 나중에 교체하는 점진 전략. (FTXUI 개념: `Herbarium/wip/ftxui.md`)
- **#4 leaves 구조**: "초안으로 가자" 합의에 따라 CLI11 파싱을 `glass/main.cpp`에 인라인으로 빠르게 배선, `ArgParser`/`Dispatcher` 클래스 추출은 04 리팩터/복기로 보류.
- **#5 JSON / JSON Schema**: 용도(Command/Result 직렬화 명세, 미래 LLM Function Calling 어댑터용)가 현재 CLI 슬라이스 밖이라 초안 우선으로 밀려남.

> 복기 교훈: 의식적 보류(#3~#5)도 02 handoff에 기록됐어야 추적성이 유지된다. 03 스킬 G12를 "이탈뿐 아니라 의식적 보류도 기록"으로 보강할지 검토 대상.
